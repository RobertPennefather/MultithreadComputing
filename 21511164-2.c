...
int main(int argc, char *argv[]) {
	...
	int numtasks, taskid, dest, offset, i, j, tag1, tag2, tag3, source, chunksize, myblocks_size, block_size;
	MPI_Status status;
	...
	chunksize = (ROW_SIZE / numtasks)+1;
	tag1 = 1;
	tag2 = 2;
	tag3 = 3;

	//Create unique data type
	MPI_Datatype blocktype, oldtypes[2];
	MPI_Aint offsets[2], extent, lb;
	int blockcounts[2];

	//For the four ints in row_id + col_id
	offsets[0] = 0;
	oldtypes[0] = MPI_INT;
	blockcounts[0] = BLOCK_SIZE + 1;

	//For the double signature
	MPI_Type_get_extent(MPI_INT, &lb, &extent);
	offsets[1] = (BLOCK_SIZE + 1) * extent;
	oldtypes[1] = MPI_DOUBLE;
	blockcounts[1] = 1;

	MPI_Type_create_struct(2, blockcounts, offsets, oldtypes, &blocktype);
	...
	if (taskid == MASTER){
		...
		struct Block* blocks = malloc(myblocks_size * sizeof(struct Block));
		...
		for (i = 0; i < chunksize; i++){
			...
			for(j = 0; j < mini_blocks_size; j++){
				myblocks_size++;
				blocks = realloc(blocks, myblocks_size * sizeof(struct Block));
				blocks[myblocks_size-1] = mini_blocks[j];
			}
			//Used for testing to display what column each thread is working on
			printf("%d(%d) - %d\n",i, taskid, myblocks_size);
		}

		//Put main into full array
		int storedSize = blockArray_size;
		blockArray_size += myblocks_size;
		blockArray = realloc(blockArray, blockArray_size * sizeof(struct Block));
		for(i=0; i<myblocks_size; i++){
			blockArray[storedSize+i] = blocks[i];
		}

		/* Wait to receive results from each task and create total array of blocks */
		for (i=1; i<numtasks; i++) {
			source = i;
			MPI_Recv(&offset, 1, MPI_INT, source, tag1, MPI_COMM_WORLD, &status);
			MPI_Recv(&myblocks_size, 1, MPI_INT, source, tag2, MPI_COMM_WORLD, &status);

			int storedSize = blockArray_size;
			blockArray_size += myblocks_size;
			blockArray = realloc(blockArray, blockArray_size * sizeof(struct Block));

			MPI_Recv(&blockArray[storedSize], myblocks_size, blocktype, source, tag3, MPI_COMM_WORLD, &status);
		}
	...
	if (taskid > MASTER) {
		...
		dest = MASTER;
		MPI_Send(&offset, 1, MPI_INT, dest, tag1, MPI_COMM_WORLD);
		MPI_Send(&myblocks_size, 1, MPI_INT, dest, tag2, MPI_COMM_WORLD);
		MPI_Send(&blocks[0], myblocks_size, blocktype, dest, tag3, MPI_COMM_WORLD);
